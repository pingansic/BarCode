...
public class TestCamera extends Activity {
	...
	class MyTimerTask extends TimerTask {
		...
		public void run() {
			if (dstLeft == 0) {// 只赋值一次
				...
				Log.i("getWidth", getWindowManager().getDefaultDisplay()
						.getWidth() + "");// 480
				...
				Log.i("getHeight", getWindowManager().getDefaultDisplay()
						.getHeight() + "");//
	...
	private Camera.PreviewCallback previewCallback = new Camera.PreviewCallback() {
		...
		public void onPreviewFrame(byte[] data, Camera arg1) {
			...
			// 显示灰度图
			/**
			 * imageView的setImageResource和setImageBitmap这两种方法有什么区别，图片资源就在res下，
			 * 为imageView赋值时用哪种方法? 第一种setImageResource
			 * 是从资源drawable中通过资源id找到文件转成可绘制对象drawable 然后绘制。这个方法会自动适配分辨率。
			 * 适用于不频繁设置图片图片资源不会太大的情况。 但是对于大图片时或者你需要不断的重复的设置图片
			 * 调用这个方法生成的drawable里一样会生成一个bitmap对象 因为bitmap是通过bitmapfactory生成的
			 * 有一部分要调用C库所以需要开辟一部分native本地内存空间以及一部分jvm的内存空间。 而native本地内存空间是C开辟的
			 * jvm的gc垃圾回收器是回收不了这部分空间的，这个时候如果你频繁的调用setImageResource 且没有手动调recycle
			 * native的内存空间很难被释放掉。jvm的内存也不会及时得到回收这样就相当容易导致内存溢出。而setImageBitmap
			 * 当你需要频繁设置大图片时 通过bitmapfactory生成bitmap然后设置
			 * 然后每次设置前将之前的bitmap手动掉recycle 置为可回收状态 这样很大程度能防止 内存泄露溢出所以看你的需求
			 * 你的图片是不频繁设置且不会太大就用第一种 如果需求不断的重复更新设置那最好用第二个并且记住手动及时回收后再设置
			...
			try {
				...
				String strResult = "码类别:"
						+ result.getBarcodeFormat().toString() + "结果："
						+ result.getText();
